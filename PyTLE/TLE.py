...
class tle_class:
    ...
    def _set_tle_fields(self):
        map(lambda x: setattr(self, x, None), tle_fields['line1'].keys())
        map(lambda x: setattr(self, x, None),
        tle_fields['line2'].keys())
    ...
    def _derive_dates(self):
        ...
        if self.epoch_year <= 57:
            self.epoch_full_year = self.epoch_year + 2000
        else:
            self.epoch_full_year = self.epoch_year + 1900
                        # try: 
            self._astro_date = astro_date( 
                tle_epoch.year_day_to_datetime( self.epoch_year, self.epoch_day) )
